...
def init_commands(cardinal: Cardinal, *args):
    ...
    def send_orders(m: telebot.types.Message):
        ...
        orders[-1] = orders[-1].rstrip(", ")
        l = ["Здравствуйте!\n\nПрошу подтвердить выполнение следующих заказов:\n", *orders, "\n\nЗаранее благодарю,\nС уважением."]
        result_msgs = utils.split_by_limit(l, limit=4096-len("<code></code>"))
        for i, result_msg in enumerate(result_msgs):
            if i == 0:
                bot.edit_message_text(f"<code>{utils.escape(result_msg)}</code>", new_mes.chat.id, new_mes.id)
            else:
                bot.send_message(new_mes.chat.id, f"<code>{utils.escape(result_msg)}</code>")
